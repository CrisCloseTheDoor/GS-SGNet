...
sys.path.append("D:\\方向：交通轨迹\\★代码\\SGNet和trajectron++\\Trajectron-plus-plus\\trajectron")
# from .orig_trajectron import NodeTypeDataset
import numpy as np
import torch
...
class ETHUCYDataLayerWithNeighbor(data.Dataset):
    ...
    def __init__(self, args, split):
        ...
        hyperparams['state'] = {
            'PEDESTRIAN': {'position': ['x', 'y'], 'velocity': ['x', 'y'], 'acceleration': ['x', 'y']}}
        ...
        if split == 'train':
            ...
            f = open(os.path.join(
                "D:\\方向：交通轨迹\\★代码\\SGNet和trajectron++\\Trajectron-plus-plus\\experiments\\" + processed_dir_name,
                args.data_root + '_train.pkl'), 'rb')
        elif split == 'val':
            # f = open(os.path.join(args.eth_root, args.data_root, 'val', args.data_root+'_val.pkl'), 'rb')
            f = open(os.path.join(
                "D:\\方向：交通轨迹\\★代码\\SGNet和trajectron++\\Trajectron-plus-plus\\experiments\\" + processed_dir_name,
                args.data_root + '_val.pkl'), 'rb')
        elif split == 'test':
            # f = open(os.path.join(args.eth_root, args.data_root, 'test', args.data_root+'_test.pkl'), 'rb')
            f = open(os.path.join(
                "D:\\方向：交通轨迹\\★代码\\SGNet和trajectron++\\Trajectron-plus-plus\\experiments\\" + processed_dir_name,
                args.data_root + '_test.pkl'), 'rb')
        else:
            raise ValueError()
        ...
        if args.modified_preprocess:
            from .modified_preprocess_trajectron import NodeTypeDataset
        else:
            from .nb_simple_trajectron import NodeTypeDataset
        ...
        for index in range(len(self.dataset)):
            first_history_index = self.dataset.__getitem__(index, get_first_idx_only=True)
    ...
    def __getitem__(self, index):
        ...
        ret = {
            ...
            'neighbors_idx_start': None, # 维护一个索引表
            'neighbors_idx_end': None,
        }
        ...
        for idx in indices:
            ...
            ret['target_y'].append(torch.as_tensor(this_ret['target_y']).type(torch.FloatTensor)) # already relative to every step's state in obs
            ret['target_y_st'].append(this_ret['target_y_st'])
            ...
            for key, _ in this_ret['neighbors_data'].items():
                if len(this_ret['neighbors_data'][key]):
                    ret['neighbors_data'].append(torch.stack(this_ret['neighbors_data'][key], dim=0))  # peds, t_obs, dim_6

                num_neighbors.append(len(this_ret['neighbors_data'][key]))
                break # only use pedestrain
        num_neighbors_cumsum = torch.cumsum(torch.as_tensor(num_neighbors).type(torch.LongTensor), dim=0)
        neighbors_idx_start = torch.zeros(num_neighbors_cumsum.shape[0]).type_as(num_neighbors_cumsum)
        neighbors_idx_start[1:] = num_neighbors_cumsum[:-1]
        neighbors_idx_end = num_neighbors_cumsum.clone()

        ret['input_x'] = torch.stack(ret['input_x'])
        ret['input_x_st'] = torch.stack(ret['input_x_st'])
        ret['target_y'] = torch.stack(ret['target_y'])
        ret['target_y_st'] = torch.stack(ret['target_y_st'])

        ret['first_history_index'] = torch.stack(ret['first_history_index'])
        # to locate image
        # ret['scene_name'] = ret['scene_name']
        # ret['timestep'] = ret['timestep']
        ret['neighbors_data'] = torch.cat(ret['neighbors_data'], dim=0)  # peds(all), t_obs, dim
        ret['neighbors_idx_start'] = neighbors_idx_start
        ret['neighbors_idx_end'] = neighbors_idx_end

        return ret

    def getitem_one(self, index):
        first_history_index, x_t, y_t, x_st_t, y_st_t, neighbors_data_st, neighbors_data, \
            neighbors_edge_value, robot_traj_st_t, map_tuple = self.dataset.__getitem__(index)
        ret = {}
        ...
        ret['neighbors_data'] = neighbors_data
        return ret

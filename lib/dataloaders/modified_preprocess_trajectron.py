...
def get_node_timestep_data(env, scene, t, node, state, pred_state,
    ...
    # Neighbors
    # modify 1: delete convolve step, only use cur step info in adj_mat and weight_mat
    hyperparams['edge_addition_filter'] = []
    hyperparams['edge_removal_filter'] = []
    # # modify 2: attn radius: 3m -> 10m
    # for k, v in env.attention_radius.items():
    #     if k[0].name == 'PEDESTRIAN' and k[1].name == 'PEDESTRIAN':
    #         env.attention_radius[k] = 10.0
    ...
    return (first_history_index, x_t, y_t, x_st_t, y_st_t, None, neighbors_data,
            None, None, None)
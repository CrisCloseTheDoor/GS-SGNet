...
def get_node_timestep_data(env, scene, t, node, state, pred_state,
    ...
    if hyperparams['edge_encoding']:
        ...
        for edge_type in edge_types:
            ...
            if hyperparams['dynamic_edges'] == 'yes':
                ...
                # We get the edge masks for the current node throughout the observation phrase
                for t_scene_graph in temporal_scene_graph:
                    edge_masks = torch.tensor(t_scene_graph.get_edge_weight(node, neighbors_list=connected_nodes), dtype=torch.float)
                    neighbors_edge_value[edge_type].append(edge_masks)
                try:
                    neighbors_edge_value[edge_type] = torch.stack(neighbors_edge_value[edge_type], dim=0)
                except RuntimeError:
                    pass
    ...
    return (first_history_index, x_t, y_t, x_st_t, y_st_t, neighbors_data_st, neighbors_data,

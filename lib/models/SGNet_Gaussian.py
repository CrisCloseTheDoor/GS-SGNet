...
class SGNet_Gaussian(nn.Module):
    def __init__(self, args):
        super(SGNet_Gaussian, self).__init__()

        self.hidden_size = args.hidden_size
        self.sample_method = args.sample_method
        self.output_dim = 5
        self.with_neighbors = args.with_neighbors
        self.N = args.K  # 20
        ...
        if self.dataset in ['JAAD','PIE']:
            ...
            self.regressor = nn.Sequential(nn.Linear(self.hidden_size, 
                                                        self.pred_dim))
            # add
            if self.sample_method in ['mc', 'qmc', 'npsn', 'npsn_orig_ablation', 'npsn_ablation']:
                self.dec_hidden2distr = nn.Sequential(
                    nn.Linear(self.hidden_size, self.hidden_size),
                    nn.ReLU(inplace=True),
                    nn.Linear(self.hidden_size, 5))
            elif self.sample_method == 'multi_predictor':
                self.dec_pos_predictor = nn.ModuleList(
                    [nn.Sequential(nn.Linear(self.hidden_size, self.hidden_size),
                    nn.ReLU(inplace=True),
                    nn.Linear(self.hidden_size, self.pred_dim)) for _ in range(self.N)]
                )
                for m in self.dec_pos_predictor.modules():
    ...
    def decoder(self, dec_hidden, goal_for_dec):
        ...
        dec_traj = dec_hidden.new_zeros(dec_hidden.size(0), self.dec_steps, self.N, self.pred_dim)
        distribution_param = dec_hidden.new_zeros(dec_hidden.size(0), self.dec_steps, self.output_dim)
        for dec_step in range(self.dec_steps):
            ...
            dec_hidden = self.dec_cell(dec_input, dec_hidden) # bs, hidden_size
            if self.sample_method == 'multi_predictor':
                for i in range(self.N):
                    dec_traj[:,dec_step,i, :] = self.dec_pos_predictor[i](dec_hidden)
            else:
                distribution_param[:, dec_step, :] = self.dec_hidden2distr(dec_hidden) # bs, 5
    ...
    def encoder(self, traj_input, flow_input=None, start_index = 0):
        ...
        all_dec_distr = traj_input.new_zeros(traj_input.size(0), self.enc_steps, self.dec_steps, self.output_dim)
        all_dec_traj = traj_input.new_zeros(traj_input.size(0), self.enc_steps, self.dec_steps, self.N, self.pred_dim)
        ...
        for enc_step in range(start_index, self.enc_steps):
            ...
            dec_res = self.decoder(dec_hidden, goal_for_dec)

            # output 
            all_goal_traj[:,enc_step,:,:] = goal_traj
            if self.sample_method == 'multi_predictor':
                all_dec_traj[:,enc_step,:,:] = dec_res
            else:
                all_dec_distr[:,enc_step,:,:] = dec_res

        if self.sample_method == 'multi_predictor':
            return all_goal_traj, all_dec_traj, enc_hidden
        return all_goal_traj, all_dec_distr, enc_hidden
            

    def forward(self, inputs, start_index = 0):
        if self.dataset in ['JAAD','PIE']:
            traj_input = self.feature_extractor(inputs)
            all_goal_traj, all_dec_traj, all_enc_hidden = self.encoder(traj_input)
            ...
            all_goal_traj, all_dec_traj, all_enc_hidden = self.encoder(traj_input, None, start_index)
            return all_goal_traj, all_dec_traj, all_enc_hidden
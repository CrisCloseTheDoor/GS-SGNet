...
def train(epoch, model, train_gen, criterion, optimizer, lr_scheduler, device):
    model.train() # Sets the module in training mode.
    num_batches = len(train_gen)
    epoch_per_Iter = 1.0 / num_batches
    epoch_plus_CurIter = epoch
    count = 0
    total_goal_loss = 0
    total_dec_loss = 0
    loader = tqdm(train_gen, total=len(train_gen),  desc='train')
    with torch.set_grad_enabled(True):
        for batch_idx, data in enumerate(loader):
            ...
            neighbors_data_st = data['neighbors_data_st'].to(device)
            neighbors_edge_value = data['neighbors_edge_value'].to(device)
            neighbors_idx_start = data['neighbors_idx_start'].to(device)
            neighbors_idx_end = data['neighbors_idx_end'].to(device)
            count += batch_size
            ...
            all_goal_traj, all_dec_traj = model(input_traj, first_history_index[0], neighbors_data_st,
                                                neighbors_edge_value, neighbors_idx_start, neighbors_idx_end)
...
def test(model, test_gen, device, use_pixel_sys=False):
    total_goal_loss = 0
    total_dec_loss = 0
    ADE_08 = 0
    ADE_12 = 0 
    ...
    count = 0
    model.eval()
    loader = tqdm(test_gen, total=len(test_gen), desc='test')
    with torch.set_grad_enabled(False):
        for batch_idx, data in enumerate(loader):#for batch_idx, data in enumerate(val_gen):

...
def test_output_traj(model, test_gen, device):
    # output traj
    all_obs = []
    all_gt = []
    all_preds = []
    ...
    with torch.set_grad_enabled(False):
        for batch_idx, data in enumerate(loader):  # for batch_idx, data in enumerate(val_gen):
            ...
            all_goal_traj, cvae_dec_traj, KLD_loss, _ = model(input_traj, map_mask=None, targets=None,
                                                              start_index=first_history_index, training=False)

            all_obs.append(input_traj[:, :, :2])
            all_gt.append(target_traj[:, -1, ...]+input_traj[:, -1, :2].unsqueeze(1))
            all_preds.append(cvae_dec_traj[:, -1, ...]+input_traj[:, -1, :2].unsqueeze(1).unsqueeze(1))
    ...
    # print("Test Loss %4f\n" % (test_loss))
    # print("ADE_08: %4f;  FDE_08: %4f;  ADE_12: %4f;   FDE_12: %4f\n" % (ADE_08, FDE_08, ADE_12, FDE_12))
    return ADE_08, FDE_08, ADE_12, FDE_12, trajectory_results

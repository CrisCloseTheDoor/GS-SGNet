...
from lib.utils.ethucy_train_utils_cvae import train, val, test, test_output_traj
import pickle
...
def main(args):
    ...
    if osp.isdir(args.pretrained_dir):
        checkpoint = torch.load(osp.join(args.pretrained_dir, args.dataset.lower()+'.pth'))
        model.load_state_dict(checkpoint['model_state_dict'], strict=True)
        del checkpoint
        print("load from", osp.join(args.pretrained_dir, args.dataset.lower()+'.pth'))
        ...
        print("Warning: No checkpoint is loaded, will use random initial model!")
    ...
    # test
    # ADE_08, FDE_08, ADE_12, FDE_12 = test(model, test_gen, criterion, device)
    ADE_08, FDE_08, ADE_12, FDE_12, trajectory_results = test_output_traj(model, test_gen, device)
